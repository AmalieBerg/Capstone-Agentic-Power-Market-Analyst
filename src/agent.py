...
def _build_agent_prompt(question: str, context_text: str) -> str:
    return (
        ...
        "When you use a live tool result, state in plain language that it is "
        "current/live and give its timestamp naturally in the sentence (e.g. "
        "'as of 08:00 UTC today'), never using bracket or special-character "
        "markers. Keep live figures clearly separate from dated source "
        "citations so the two time frames are never conflated. Answer "
        "concisely, at most ~150 words. If neither sources nor tools answer "
        "the question, say so.\n\n"
        f"Sources:\n{context_text}\n\n"
        f"Question: {question}\n\n"
